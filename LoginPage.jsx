...
const LoginPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        await axios.post("http://localhost:8080/employee/login", userData)
            .then((res) => { console.log(res.data) })
            .then(() => { navigate("/LoginPage/HomePage", { state: { email: email } }) })
            ...
            <div className="max-w-md w-full space-y-8">
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            <input
                                ...
                                name="remember-me"
                                type="checkbox"
                                className="h-4 w-4 text-gradient-from-red-700-to-black border-gray-300 rounded"
                            />
                            <label htmlFor="remember-me" className="ml-2 block text-sm text-gradient-from-red-700-to-black">
                                Are u a Manager?
                            </label>
                        </div>

                        <div className="text-sm">
                            <a href="/SignupPage" className="font-medium text-gradient-from-red-700-to-black hover:text-red-900">
                    ...
                    <div>
                        <button
                            type="submit"
                            className={`group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium bg-gradient-to-r from-red-700 to-black hover:from-bg-red-900 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-500 ease-in-out transform ${isLoading ? 'opacity-50 cursor-not-allowed' : ''
                                }`}
                            disabled={isLoading}
                        >
